...
class InsufficientProfitError(TransactionError):
    """Raised when a trade is not profitable enough to proceed."""
    def __init__(self, message: str = "Insufficient profit for trade", required_profit: float = 0, actual_profit: float = 0):
        details = {
            "required_profit": required_profit,
            "actual_profit": actual_profit,
            "reason": "Trade does not meet minimum profit threshold"
        }
        super().__init__(message, details=details)
...
class FlashbotsError(ON1BuilderError):
    """Raised for errors related to Flashbots relay or bundle submission."""
    def __init__(self, message: str = "Flashbots error", details: Optional[Dict[str, Any]] = None):
        super().__init__(message, details)