...
class GlobalSettings(BaseModel):
    ...
    mev_share_enabled: bool = Field(default=True)
    bundle_simulation_enabled: bool = Field(default=True)
    advanced_arbitrage_enabled: bool = Field(default=True)
    profit_optimization_enabled: bool = Field(default=True)
    
    # Performance thresholds
    min_roi_percentage: float = Field(default=5.0, ge=0)  # Minimum 5% ROI
    max_bundle_timeout_blocks: int = Field(default=2, ge=1, le=5)
    simulation_timeout_seconds: float = Field(default=5.0, gt=0)
    
    # Gas optimization
    gas_price_optimization_enabled: bool = Field(default=True)
    gas_price_history_size: int = Field(default=1000, gt=0)
    gas_market_analysis_enabled: bool = Field(default=True)
