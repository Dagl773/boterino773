...
class SafetyGuard:
    ...
    def __init__(self, web3_factory: Web3Factory):
        self.web3_factory = web3_factory
        self.logger = logging.getLogger(__name__)
        
        # Risk control parameters
        self.max_gas_price_gwei = 100  # Maximum gas price in gwei
        self.emergency_pause = False  # Emergency pause flag
        self.max_failed_bundles = 5  # Maximum failed bundles before alert
        self.failed_bundle_window = 300  # Time window in seconds for failed bundle tracking
        
        # Failed bundle tracking
        self.failed_bundles: List[Dict] = []
    ...
    async def check_risk_controls(self, transaction_data: Dict) -> Tuple[bool, str]:
        """
        Check all risk controls before transaction execution.
        ...
        try:
            ...
            if gas_price > self.max_gas_price_gwei:
                return False, f"Gas price too high: {gas_price}gwei > {self.max_gas_price_gwei}gwei"
            
            # Check failed bundle threshold
            if self._check_failed_bundle_threshold():
                return False, "Too many failed bundles recently"
            
            # All checks passed
            return True, "Risk controls passed"
            
        except Exception as e:
            self.logger.error(f"Error in risk control check: {e}")
            return False, f"Risk control error: {e}"
    ...
    def _check_failed_bundle_threshold(self) -> bool:
        """Check if too many bundles have failed recently."""
        try:
            current_time = time.time()
            
            # Remove old failed bundles outside the window
            self.failed_bundles = [
                bundle for bundle in self.failed_bundles
                if current_time - bundle['timestamp'] < self.failed_bundle_window
            ]
            
            # Check if threshold exceeded
            if len(self.failed_bundles) >= self.max_failed_bundles:
                # Only alert once per window
                if current_time - self.last_alert_time > self.failed_bundle_window:
                    self.logger.warning(f"Failed bundle threshold exceeded: {len(self.failed_bundles)} failed bundles")
            ...
            self.logger.error(f"Error checking failed bundle threshold: {e}")
            return False
    
    def record_failed_bundle(self, bundle_hash: str, error: str):
        """Record a failed bundle for tracking."""
        try:
            ...
            self.logger.warning(f"Failed bundle recorded: {bundle_hash} - {error}")
            
        except Exception as e:
            self.logger.error(f"Error recording failed bundle: {e}")
    
    def set_emergency_pause(self, paused: bool):
        """Set emergency pause flag."""
        self.emergency_pause = paused
        status = "PAUSED" if paused else "RESUMED"
        self.logger.warning(f"Emergency pause {status}")
        
        # Send notification
        if hasattr(self, '_notification_service'):
            self._notification_service.send_alert(
                level="warning",
                message=f"Emergency pause {status.lower()}",
    ...
    def get_risk_status(self) -> Dict:
        ...
        try:
            return {
                ...
                "failed_bundles_count": len(self.failed_bundles),
                "max_failed_bundles": self.max_failed_bundles,
                "failed_bundle_window_seconds": self.failed_bundle_window,
